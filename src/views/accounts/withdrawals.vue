  ...
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      <el-form :inline="true" class="demo-form-inline">
        ...
        <el-form-item>
          <el-select clearable class="filter-item" v-model="listQuery.status" placeholder="审核状态">
      ...
      <el-table-column  align="left" label="操作" width="260">
        <template scope="scope">
          ...
          <!--<template v-if="[0, 1, 2].indexOf(scope.row.status) >= 0">
           ...
           <template v-if="[1].indexOf(scope.row.status) >= 0">
    ...
    <el-dialog title="查看" :visible.sync="dialogFormVisible" size="large">
  	...
  	<el-table  :key='tableKey' :data="dial" v-loading.body="listLoading" border fit highlight-current-row style="width:722px;margin-left:50px;">
  

      <el-table-column align="center" label="审核人"  width="180px">
        <template scope="scope">
          <span></span>
        </template>
      </el-table-column>
     ...
     <!-- audit_desc-->
      <el-table-column label="审核说明" width="180px" align="center">
        <template scope="scope">
          <span>{{scope.row.audit_desc}}</span>
        </template>
      </el-table-column>
    <!--  {{scope.row.status | checkWithdrawFilter}}-->
...
//      	console.log(index);
          ...
          detail({withdraw_id: row.id}).then(res => {
            ...
            if(d.data && d.data.withdraw){
              ...
              this.dial = d.data.withdraw.withdrawal_audit_d_t_o_s;
              console.log(this.dial);
//            self.temp.withdrawal_audit_d_t_o_s = self.temp.withdrawal_audit_d_t_o_s || {};
//            console.log( self.temp.user_bank_card_d_t_o);
//             var w = self.temp.withdrawal_audit_d_t_o_s [0];
             console.log(self.temp.withdrawal_audit_d_t_o_s);
