<template>
  <div class="app-container">
    ...
    <el-table  :key='tableKey' index="index" :data="list" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="100px" align="center" label="愿望类型">
        <template scope="scope">
          <span>{{scope.row.category | wishListFilter}}</span>
        </template>
      </el-table-column>

      <el-table-column width="100px" align="center" label="所需金额">
        <template scope="scope">
    ...
    <!-- <el-dialog :title="textMap[dialogStatus]" v-if="dialogFormVisible" size="large"> -->
      <el-form class="small-space" :model="temp" label-position="left" label-width="70px" style='width: 80%; margin-left:50px;'>
        ...
        <el-form-item label="封面图">
            <adminSingleImage v-model="temp.cover_url"></adminSingleImage>
        </el-form-item>
        <el-form-item label="愿望顶图">
          <adminImages v-model="temp.img_url"></adminImages>
        </el-form-item>
...
//        通过api里面的fetchWishById
          fetchWishById({wish_id: row.id}).then(res => {
           ...
           self.temp.img_url.splice(0,img_url.length);
            self.getList();
        },
        checkTemp(){
          var temp = this.temp;
