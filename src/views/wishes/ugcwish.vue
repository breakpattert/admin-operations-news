...
<!--这里的状态要改成处理-->
    ...
    <el-table  :key='tableKey' index="index" :data="list" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="80px" align="center" label="UGC愿望id">
...
<!--下面这些都拿掉-->
	...
	<el-table-column  align="center" label="昵称">
        <template scope="scope">
      	...
      	<el-table-column  align="center" label="愿望名称">
        <template scope="scope">
          <span>{{scope.row.wish_name}}</span>
        </template>
      </el-table-column>
  ...
  <!--        <el-button size="small" type="primary" @click="handleUpdate(scope.row)">查看</el-button>-->
       ...
       <!-- 	<template v-if="[0,1,2].indexOf(scope.row.status) == 0">
        ...
        <!--    <el-button size="small" type="primary" @click="editor(scope.row)">处理</el-button>
   				...
   				<!--	<template scope="scope">-->
  					<el-button type="small" @click="open2(scope.row)">查看</el-button>
<!--</template>-->
...
<script>
    import { ugcWishList, detail, editor,changeShelfStatus} from 'api/ugcwish';
      ...
      methods: {
       	open2(row) {
        	...
        	this.$msgbox({
          	...
          	message: h('p', null, [
            	h('i', null, '愿望标题'),
            	h('div', { style: 'padding: 10px;min-height:120px;overflow-y:auto;max-height:200px;width:90%;border:1px solid rgba(0,0,0,0.3);border-radius: 10px;' },row.wish_name),
             	h('i', null, '愿望内容 '),
            	h('div', { style: 'padding: 10px;min-height:120px;overflow-y:auto;max-height:200px;width:90%;border:1px solid rgba(0,0,0,0.3);border-radius: 10px;' },row.wish_content),
             	h('i', null, '删除原因 '),
            	h('span', { style: 'padding: 10px;min-height:120px;overflow-y:auto;max-height:200px;width:90%;border:1px solid rgba(0,0,0,0.3);border-radius: 10px;' },row.operate_desc)
          	]),
...
//            	instance.confirmButtonText = '执行中...';
        ...
        getList() {
          ...
          ugcWishList(this.listQuery).then(res => {
            var d = res.data;
...
//      },
       ...
       handleUpdate(row) {
       		this.dialogFormVisible = true;
						this.main ={
							content:row.ish_content,
							myoperate_desc:this.temp.operate_desc,
							titles:row.wish_name
						}
