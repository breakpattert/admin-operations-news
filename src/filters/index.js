...
export function feedStatusListFilter(id, reverst){
  return findAndReverstIt(feedStatusList, id, reverst)
}
...
export function homebannerStatusListFilter(id, reverst){
  return findAndReverstIt(homebannerStatusList, id, reverst)
}
function findStatus(arr, status){
  var rst = arr.find(item => item.status == status);
  if(rst){
    return rst.name;
  }else{
    return status;
...
// 审核状态 filter，checkWithdrawStatusList审核的状态码
export function checkWithdrawFilter(status){
return findStatus(checkWithdrawStatusList, status);
return checkWithdrawStatusList.find(item => item.status == status).name
//return findIt(checkWithdrawStatusList,status );
}
...
//findIt对传入的arr和id做一个判断处理的封装
export function wishListFilter(id){
  return findIt(wishList, id);
}
export function appListFilter(id){
  return findIt(appList, id);
}
const appList = [{
  id: 0,
  name: '安卓'
...
export function statislistFilter(id){
  return findIt(statislist, id);
}
// 交易方式
const paymentList = [{
  id: 0,
